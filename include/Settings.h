...
namespace EdgeVO{
    namespace Settings{
        ...
        enum SELECT_DATASET
        {
            ...
            rgbd_dataset_freiburg3_cabinet_validation = 12,
            rgbd_dataset_freiburg3_large_cabinet = 13
            //* yzhen105: add new dataset's index here (start from 12)
        };
        ...
        {   std::string("rgbd_dataset_freiburg1_xyz"), 
            ...
            std::string("rgbd_dataset_freiburg3_cabinet_validation"),
            std::string("rgbd_dataset_freiburg3_large_cabinet")
            //* yzhen105: add new dataset's name here (start from 12)
        };

        const int DATASET_NUMBER = 13; //* yzhen105: choose the dataset to compile here, input index

        const int KEYFRAME_INTERVAL(3); //* yzhen105 should always be 3 level 

        const int PYRAMID_DEPTH(3); //* yzhen105 should always be 3 level 
        ...
        const std::string DATASET_DIRECTORY( std::string("/users/yzhen105/data/yzhen105/datasets/") + DATASET + std::string("/") );
        //* yzhen105: diectory modified to match the folder using
        const std::string ASSOC_FILE( DATASET_DIRECTORY + std::string("associate.txt") );
        //* yzhen105: modified to match the folder using
        ...
        {   std::string("python evaluate_rpe.py groundtruth_fr1xyz.txt rgbd_dataset_freiburg1_xyz_results.txt --fixed_delta --delta 1 --verbose") , 
            ...
            std::string("python evaluate_rpe.py groundtruth_fr3cab.txt rgbd_dataset_freiburg3_cabinet_validation_results.txt --fixed_delta --delta 1 --verbose"),
            std::string("python evaluate_rpe.py groundtruth_fr3largcab.txt rgbd_dataset_freiburg3_large_cabinet_results.txt --fixed_delta --delta 1 --verbose")
            };
        ...
        {   std::string("python evaluate_ate.py groundtruth_fr1xyz.txt rgbd_dataset_freiburg1_xyz_results.txt --verbose") , 
            ...
            std::string("python evaluate_ate.py groundtruth_fr3cab.txt rgbd_dataset_freiburg3_cabinet_validation_results.txt --verbose"),
            std::string("python evaluate_ate.py groundtruth_fr3largcab.txt rgbd_dataset_freiburg3_large_cabinet_results.txt --verbose")
            };
